 ...
 * <p>
 * We manage the Camera and GLES rendering from a dedicated thread. We don't
 * animate anything, so we don't need a Choreographer heartbeat -- just redraw
 * when we get a new frame from the camera or the user has caused a change in
 * size or position.
 * <p>
 * The Camera needs to follow the activity pause/resume cycle so we don't keep
 * it locked while we're in the background. Also, for power reasons, we don't
 * want to keep getting frames when the screen is off. As noted in
 * http://source.android.com/devices/graphics/architecture.html#activity the
 * Surface lifecycle isn't quite the same as the activity's. We follow approach
 * #1.
 * <p>
 * The tricky part about the lifecycle is that our SurfaceView's Surface can
 * outlive the Activity, and we can get surface callbacks while paused, so we
 * need to keep track of it in a static variable and be prepared for calls at
 * odd times.
 * <p>
 * The zoom, size, and rotate values are determined by the values stored in the
 * "seek bars" (sliders). When the device is rotated, the Activity is paused and
 * resumed, but the controls retain their value, which is kind of nice. The
 * position, set by touch, is lost on rotation.
 * <p>
 ...
 * <li>The render thread converts the percent to something concrete (e.g. size
 * in pixels). The rect geometry is updated.
 * <li>(For most things) The values computed by the render thread are sent back
...
public class TextureFromCameraActivity extends Activity implements SurfaceHolder.Callback, SeekBar.OnSeekBarChangeListener {
	...
	private static final int REQ_CAMERA_WIDTH = 640;
	private static final int REQ_CAMERA_HEIGHT = 368;
	private static final int REQ_CAMERA_FPS = 30;

	// The holder for our SurfaceView. The Surface can outlive the Activity
	// (e.g. when
	// the screen is turned off and back on with the power button).
	//
	// This becomes non-null after the surfaceCreated() callback is called, and
	// gets set
	// to null when surfaceDestroyed() is called.
	private static SurfaceHolder sSurfaceHolder;

	// Thread that handles rendering and controls the camera. Started in
	// onResume(),
	// stopped in onPause().
	...
	// These values are passed to us by the camera/render thread, and displayed
	// in the UI.
	// We could also just peek at the values in the RenderThread object, but
	// we'd need to
	...
	public void surfaceCreated(SurfaceHolder holder) {
		...
		if (mRenderThread != null) {
			// Normal case -- render thread is running, tell it about the new
			// surface.
			RenderHandler rh = mRenderThread.getHandler();
			rh.sendSurfaceAvailable(holder, true);
		} else {
			// Sometimes see this on 4.4.x N5: power off, power on, unlock, with
			// device in
			// landscape and a lock screen that requires portrait. The
			// surface-created
			// message is showing up after onPause().
			//
			// Chances are good that the surface will be destroyed before the
			// activity is
			// unpaused, but we track it anyway. If the activity is un-paused
			// and we start
			// the RenderThread, the SurfaceHolder will be passed in right after
			// the thread
			// is created.
	...
	public void surfaceDestroyed(SurfaceHolder holder) {
		// In theory we should tell the RenderThread that the surface has been
		// destroyed.
		if (mRenderThread != null) {
			RenderHandler rh = mRenderThread.getHandler();
	...
	public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
		if (mRenderThread == null) {
			// Could happen if we programmatically update the values after
			// setting a listener
			// but before starting the thread. Also, easy to cause this by
			// scrubbing the seek
		...
		// If we're getting preview frames quickly enough we don't really need
		// this, but
		// we don't want to have chunky-looking resize movement if the camera is
		// slow.
		// OTOH, if we get the updates too quickly (60fps camera?), this could
		// jam us
	...
	public boolean onTouchEvent(MotionEvent e) {
		...
		case MotionEvent.ACTION_DOWN:
			// Log.v(TAG, "onTouchEvent act=" + e.getAction() + " x=" + x +
			// " y=" + y);
			if (mRenderThread != null) {
				RenderHandler rh = mRenderThread.getHandler();
				rh.sendPosition((int) x, (int) y);

				// Forcing a redraw can cause sluggish-looking behavior if the
				// touch
	 ...
	 * <p>
	 * Receives messages from the renderer thread with UI-related updates, like
	 * the camera parameters (which we show in a text message on screen).
	 */
	private static class MainHandler extends Handler {
		...
		public void sendCameraParams(int width, int height, float fps) {
			// The right way to do this is to bundle them up into an object. The
			// lazy
	...
	private static class RenderThread extends Thread implements SurfaceTexture.OnFrameAvailableListener {
		...
		// used from
		// UI thread, so we need to declare it volatile to ensure the UI thread
		// sees a fully
		// constructed object.
		...
		/**
		 * Constructor. Pass in the MainHandler, which allows us to send stuff
		...
		public void run() {
			...
			// Prepare EGL and open the camera before we start handling
			// messages.
		...
		/**
		 * Returns the render thread's Handler. This may be called from any
		 * thread.
		 */
		...
		/**
		 * Handles the surface-created callback from SurfaceView. Prepares GLES
		 * and the Surface.
		 */
		private void surfaceAvailable(SurfaceHolder holder, boolean newSurface) {
			...
			// Create and configure the SurfaceTexture, which will receive
			// frames from the
			...
			if (!newSurface) {
				...
				//
				// We could also just call this unconditionally, and perhaps do
				// an unnecessary
		...
		/**
		 * Releases most of the GL resources we currently hold (anything
		 * allocated by surfaceAvailable()).
		 * <p>
		 ...
		 * <p>
		 * We always receive surfaceChanged() after surfaceCreated(), but
		 * surfaceAvailable() could also be called with a Surface created on a
		 * previous run. So this may not be called.
		 */
		...
		private void surfaceDestroyed() {
			// In practice this never appears to be called -- the activity is
			// always paused
			// before the surface is destroyed. In theory it could be called
			// though.
		...
		/**
		 * Updates the geometry of mRect, based on the size of the window and
		 * the current values set by the UI.
		 */
		private void updateGeometry() {
			...
			// Max scale is a bit larger than the screen, so we can show
			// over-size.
		...
		private void frameAvailable() {
			long startUpdateTexImageTime = System.currentTimeMillis();
			mCameraTexture.updateTexImage();
			long endUpdateTexImageTime = System.currentTimeMillis();
			draw();
			long endDrawTime = System.currentTimeMillis();
			Log.i(TAG, "updateTexImage cost: " + (endUpdateTexImageTime - startUpdateTexImageTime) + " draw cost: "
					+ (endDrawTime - endUpdateTexImageTime));
		}
		...
		/**
		 * Opens a camera, and attempts to establish preview mode at the
		 * specified width and height with a fixed frame rate.
		 * <p>
		...
		private void openCamera(int desiredWidth, int desiredHeight, int desiredFps) {
			...
			int thousandFps = 24 * 1000;//CameraUtils.chooseFixedPreviewFps(parms, desiredFps * 1000);
			parms.setPreviewFpsRange(thousandFps, 30 * 1000);
			Log.i(TAG, "thousandFps = " + thousandFps);
			// Give the camera a hint that we're recording video. This can have
			// a big
			...
			Log.i(TAG, " PreviewFpsRange @[" + (fpsRange[0] / 1000.0) + " - " + (fpsRange[1] / 1000.0) + "] fps");
	...
	/**
	 * Handler for RenderThread. Used for messages sent from the UI thread to
	 * the render thread.
	 * <p>
	 * The object is created on the render thread, and the various "send"
	...
	private static class RenderHandler extends Handler {
		...
		// This shouldn't need to be a weak ref, since we'll go away when the
		// Looper quits,
		...
		/**
		 * Sends the "surface available" message. If the surface was newly
		 * created (i.e. this is called from surfaceCreated()), set newSurface
		 * to true. If this is being called during Activity startup for a
		 * previously-existing surface, set newSurface to false.
		 * <p>
		 * The flag tells the caller whether or not it can expect a
		 * surfaceChanged() to arrive very soon.
		 * <p>
		 * Call from UI thread.
		 */
